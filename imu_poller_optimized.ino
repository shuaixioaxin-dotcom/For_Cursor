...
// 增加总线静默时间（约4个字符时间 @ 921600 baud ≈ 44us，加上安全余量）
static const uint32_t BUS_SILENCE_US = 800;
// 增加TX使能延时，确保RS485芯片完全切换
static const uint32_t TX_ENABLE_DELAY_US = 100;
// 增加TX禁用延时，确保最后一个字节完全发送
static const uint32_t TX_DISABLE_DELAY_US = 150;
// 帧间最小延时（毫秒），确保从设备有足够恢复时间
static const uint32_t MIN_INTER_FRAME_DELAY_MS = 2;

// ================= 优化2: 重试机制 =================
static const uint8_t MAX_RETRY_COUNT = 2;  // 每个请求最大重试次数
static const uint32_t RETRY_DELAY_MS = 5;  // 重试前等待时间
...
uint8_t response_buf[RESPONSE_LEN + 16];  // 优化3：增加缓冲区余量
uint16_t response_pos = 0;

// 重试相关变量
uint8_t current_retry_count = 0;
uint32_t last_request_end_ms = 0;  // 上次请求完成时间
...
uint32_t retry_count = 0;         // 新增：重试计数
...
static const bool DEBUG_RETRY = true;  // 显示重试信息
...
bool parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len, ImuData &out) {
  ...
  if (buf[0] != slave_id || buf[1] != MODBUS_FUNC_READ_HREG) {
    if (DEBUG_PARSING) {
      Serial.printf("Parse fail: id=%02X (exp %02X), func=%02X\n", buf[0], slave_id, buf[1]);
    }
    return false;
  }
...
void recordFailure(uint8_t idx) {
  if (imu_data[idx].fail_streak < 3) {
    imu_data[idx].fail_streak++;
  }
  // 降低退避时间，使用更温和的策略
  uint32_t backoff = 10 << imu_data[idx].fail_streak;  // 20, 40, 80ms
...
void reportStatistics() {
  ...
  // 计算成功率
  uint32_t total_attempts = success_count + fail_count + timeout_count;
  if (total_attempts > 0) {
...
void setup() {
  ...
  // 优化7：配置串口缓冲区大小
  Serial2.setRxBufferSize(256);  // 增加接收缓冲区
  ...
  Serial.println("#   1. Increased RS485 switching delays");
  Serial.println("#   2. Added retry mechanism (max 2 retries per request)");
  Serial.println("#   3. Improved RX buffer clearing");
  Serial.println("#   4. Added inter-frame delay");
  Serial.println("#   5. Increased RX buffer size");
  ...
  switch (current_state) {
    case STATE_IDLE: {
      ...
      if (found_ready && busIsIdle()) {
        // 优化8：确保帧间有足够延时
        if (now_ms - last_request_end_ms >= MIN_INTER_FRAME_DELAY_MS) {
      ...
      while (Serial2.available() > 0 && response_pos < RESPONSE_LEN) {
        ...
        if (response_pos == 0) {
          if (byte_in != IMU_IDS[current_imu_index]) {
            // 跳过非目标设备的字节，但记录日志
            if (DEBUG_RAW_RESPONSES) {
              Serial.printf("# Skip byte: %02X (expected %02X)\n", 
                           byte_in, IMU_IDS[current_imu_index]);
            }
            continue;
          }
        }
        // 第二个字节：必须是功能码
        else if (response_pos == 1) {
          if (byte_in != MODBUS_FUNC_READ_HREG) {
            if (DEBUG_RAW_RESPONSES) {
              Serial.printf("# Invalid func code: %02X, reset\n", byte_in);
            }
            response_pos = 0;
            continue;
          }
        ...
        // 第三个字节：必须是正确的字节数
        else if (response_pos == 2) {
          if (byte_in != RESPONSE_BYTE_COUNT) {
            if (DEBUG_RAW_RESPONSES) {
              Serial.printf("# Invalid byte count: %d, reset\n", byte_in);
            }
            response_pos = 0;
            continue;
          }
    ...
    case STATE_PROCESSING_DATA: {
      bool success = parseResponse(IMU_IDS[current_imu_index], response_buf, response_pos, imu_data[current_imu_index]);
      ...
      if (success) {
        ...
        if (current_retry_count < MAX_RETRY_COUNT) {
          ...
          retry_count++;
          if (DEBUG_RETRY) {
            Serial.printf("# IMU %d parse fail, retry %d/%d\n", 
                         IMU_IDS[current_imu_index], current_retry_count, MAX_RETRY_COUNT);
          }
          current_state = STATE_RETRY_DELAY;
          state_start_ms = now_ms;
