...
static const uint32_t RS485_BAUD = 2000000;  // 优化：921600 → 2000000（如硬件支持）
...
static const uint16_t MODBUS_REG_COUNT = 0x000B;  // 优化：22 → 11个寄存器（仅加速度+四元数）
static const uint8_t MODBUS_REQUEST_LEN = 8;
static const uint16_t RESPONSE_BYTE_COUNT = MODBUS_REG_COUNT * 2;  // 22字节
static const uint16_t RESPONSE_LEN = 1 + 1 + 1 + RESPONSE_BYTE_COUNT + 2;  // 27字节
static const uint16_t RESPONSE_TIMEOUT_MS = 5;   // 优化：30ms → 5ms
static const uint32_t BUS_SILENCE_US = 100;      // 优化：500μs → 100μs
static const uint32_t TX_ENABLE_DELAY_US = 10;   // 优化：30μs → 10μs
static const uint32_t TX_DISABLE_DELAY_US = 20;  // 优化：60μs → 20μs
static const uint32_t FAIL_COOLDOWN_MS = 10;     // 优化：20ms → 10ms
static const uint8_t MAX_BACKOFF_SHIFT = 3;      // 优化：4 → 3
...
static const uint32_t FREQ_REPORT_INTERVAL_MS = 5000;  // 优化：1秒 → 5秒
static const uint8_t CSV_OUTPUT_DIVIDER = 10;  // 优化：每10个周期输出1次
...
static uint8_t csv_output_counter = 0;
...
static inline void zeroImu(ImuData &data) {
  data.acc[0] = 0.0f;
  data.acc[1] = 0.0f;
  data.acc[2] = 0.0f;
  data.quat[0] = 0.0f;
...
static bool parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len, ImuData &out) {
  ...
  // 读取四元数（假设紧跟加速度之后，4个寄存器，8字节）
  // 根据实际IMU数据格式调整偏移
  size_t quat_start = data_start + 14;  // 跳过加速度(6) + 其他数据(8)
  if (quat_start + 8 <= len - 2) {
    int16_t qw = static_cast<int16_t>((buf[quat_start] << 8) | buf[quat_start + 1]);
    int16_t qx = static_cast<int16_t>((buf[quat_start + 2] << 8) | buf[quat_start + 3]);
    int16_t qy = static_cast<int16_t>((buf[quat_start + 4] << 8) | buf[quat_start + 5]);
    int16_t qz = static_cast<int16_t>((buf[quat_start + 6] << 8) | buf[quat_start + 7]);
...
static void outputCycleCsv() {
  csv_output_counter++;
  if (csv_output_counter < CSV_OUTPUT_DIVIDER) {
    return;
  }
...
void setup() {
  ...
  Serial.println("# IMU Optimized (Aggressive) - Target: 400+ Hz");
  ...
  Serial.print(MODBUS_REG_COUNT);
  Serial.println(" (optimized)");
  Serial.println("# CSV order: id,accx,accy,accz,qw,qx,qy,qz (repeat)");
  Serial.print("# CSV output every ");
  Serial.print(CSV_OUTPUT_DIVIDER);
