...
static const uint32_t TX_DISABLE_DELAY_US = 80;

// ================= Reliability Tuning =================
static const uint16_t MIN_POLL_INTERVAL_MS = 2;
...
uint16_t expected_response_len = RESPONSE_LEN;
...
// Debug flags - set false to reduce output
...
static void resetResponseState() {
  response_pos = 0;
  expected_response_len = RESPONSE_LEN;
}

static bool acceptResponseByte(uint8_t byte_in, uint8_t slave_id) {
  if (response_pos == 0) {
    if (byte_in != slave_id) return false;
  ...
  if (response_pos == 2) {
    uint8_t byte_count = byte_in;
    uint16_t expected_len = static_cast<uint16_t>(byte_count) + 5;
    if (byte_count == 0 || expected_len > RESPONSE_LEN) {
      resetResponseState();
      return false;
    }
    expected_response_len = expected_len;
    response_buf[response_pos++] = byte_in;
    return true;
  }
...
static void scheduleNextPoll(uint8_t idx, uint32_t delay_ms) {
  if (delay_ms < MIN_POLL_INTERVAL_MS) {
    delay_ms = MIN_POLL_INTERVAL_MS;
  }
  imu_data[idx].next_poll_ms = millis() + delay_ms;
}
...
ParseResult parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len, ImuData &out) {
  ...
  size_t expected_len = static_cast<size_t>(byte_count) + 5;
  if (expected_len != len) {
    return PARSE_BAD_FRAME;
  }
  ...
  // Quaternion: read last 8 bytes if available
  if (byte_count >= 14) {
    size_t quat_offset = data_start + (byte_count - 8);
...
void recordSuccess(uint8_t idx) {
  ...
  if (IMU_POLL_INTERVAL_MS[idx] > 0) {
    scheduleNextPoll(idx, IMU_POLL_INTERVAL_MS[idx]);
  } else {
    scheduleNextPoll(idx, 0);
  }
}
...
void recordFailure(uint8_t idx) {
  ...
  uint32_t backoff = 20UL << imu_data[idx].fail_streak;
  scheduleNextPoll(idx, backoff);
...
void setup() {
  ...
  Serial.println("# Optimized Modbus RTU IMU Poller - Stable Version");
...
void loop() {
  ...
  switch (current_state) {
    case STATE_IDLE: {
      if (pending_retry) {
        if (now_ms >= retry_due_ms && busIsIdle()) {
    ...
    case STATE_WAITING_RESPONSE:
      ...
      while (Serial2.available() > 0 && response_pos < expected_response_len) {
        uint8_t byte_in = Serial2.read();
        last_bus_activity_us = micros();
        acceptResponseByte(byte_in, IMU_IDS[current_imu_index]);
      }

      if (response_pos >= expected_response_len && expected_response_len >= 5) {
