...
// Small sacrifice in realtime to increase stability
static const uint32_t MIN_POLL_INTERVAL_MS = 2;
static const uint8_t MAX_RETRIES = 1;
static const uint32_t RETRY_DELAY_MS = 2;
static const uint32_t INTER_BYTE_TIMEOUT_MS = 4;
...
uint32_t retry_success_count = 0;
...
static uint32_t effectivePollInterval(uint8_t idx) {
  uint32_t interval = IMU_POLL_INTERVAL_MS[idx];
  if (interval == 0 || interval < MIN_POLL_INTERVAL_MS) {
...
static ParseResult parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len, ImuData &out) {
  if (len != RESPONSE_LEN) {
...
void recordSuccess(uint8_t idx) {
  ...
  imu_data[idx].next_poll_ms = millis() + effectivePollInterval(idx);
}
...
static void scheduleRetryOrFinalize(bool is_timeout, bool is_crc) {
  if (is_timeout) {
    timeout_count++;
  } else {
    fail_count++;
  ...
  if (current_retry < MAX_RETRIES) {
    current_retry++;
    retry_count++;
    retry_ready_ms = millis() + RETRY_DELAY_MS;
    response_pos = 0;
    clearRxBuffer();
    current_state = STATE_RETRY_WAIT;
    return;
  }
...
void reportStatistics() {
  ...
  Serial.printf("Retries: %lu, Retry Success: %lu\n", retry_count, retry_success_count);
...
static bool pushByteToBuffer(uint8_t byte_in) {
  if (response_pos == 0) {
...
void loop() {
  ...
  switch (current_state) {
    ...
    case STATE_RETRY_WAIT:
      if (now_ms >= retry_ready_ms && busIsIdle()) {
        current_state = STATE_SENDING_REQUEST;
        state_start_ms = now_ms;
      }
      break;
    ...
    case STATE_WAITING_RESPONSE:
      if (response_pos > 0 && (now_ms - last_rx_ms > INTER_BYTE_TIMEOUT_MS)) {
        scheduleRetryOrFinalize(true, false);
        break;
      }
    ...
    case STATE_PROCESSING_DATA: {
      ...
      if (result == PARSE_OK) {
        ...
        scheduleRetryOrFinalize(false, result == PARSE_BAD_CRC);
        break;
      }
