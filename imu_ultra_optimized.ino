...
// ULTRA优化: 如果只需要加速度(3寄存器)和四元数(4寄存器)，可以减少到11寄存器
// 请根据实际数据布局调整。如果前11个寄存器不包含所需数据，请修改REG_START或REG_COUNT
static const uint16_t MODBUS_REG_COUNT = 0x000B;  // 11 registers (减少50%)
...
// ================= ULTRA OPTIMIZED TIMING PARAMETERS =================
// 极限优化：最小化所有延迟
static const uint16_t RESPONSE_TIMEOUT_MS = 6;      // 极限值
static const uint32_t BUS_SILENCE_US = 100;         // 极限值
static const uint32_t TX_ENABLE_DELAY_US = 5;      // 极限值  
static const uint32_t TX_DISABLE_DELAY_US = 10;    // 极限值
static const uint32_t FAIL_COOLDOWN_MS = 5;        // 极限值
static const uint8_t MAX_BACKOFF_SHIFT = 2;        // 减少回退时间
...
static const uint8_t OUTPUT_DIVIDER = 2;  // 每2个周期输出一次数据
...
static void reportFrequency() {
  ...
  if (success_count + fail_count > 0) {
    success_rate = (static_cast<float>(success_count) * 100.0f) / (success_count + fail_count);
  }
...
static void advanceImuIndex() {
  ...
  if (current_imu_index >= NUM_IMUS) {
    ...
    // 减少输出频率以提升性能
    if (++output_counter >= OUTPUT_DIVIDER) {
      output_counter = 0;
      outputCycleCsv();
    }
  }
...
void setup() {
  ...
  Serial.println("# ULTRA OPTIMIZED for 200+ Hz");
  ...
  Serial.println(OUTPUT_DIVIDER);
  Serial.println("# CSV: id,accx,accy,accz,qw,qx,qy,qz (repeat)");
