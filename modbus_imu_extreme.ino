...
// ================= 极限时序参数 =================
static const uint16_t RESPONSE_TIMEOUT_MS = 3;       // 极限: 3ms超时
static const uint32_t BUS_SILENCE_US = 50;           // 极限: 50us静默
static const uint32_t TX_ENABLE_DELAY_US = 5;        // 极限: 5us
static const uint32_t TX_DISABLE_DELAY_US = 10;      // 极限: 10us
...
static const uint32_t OUTPUT_INTERVAL_MS = 5;  // 5ms = 200Hz输出
...
static uint16_t crc16_fast(const uint8_t *data, size_t len) {
  uint16_t crc = 0xFFFF;
...
static inline bool busIsIdle() {
  return (last_bus_activity_us == 0) || 
         ((micros() - last_bus_activity_us) >= BUS_SILENCE_US);
}

static inline void zeroImu(uint8_t idx) {
  memset(&imu_data[idx], 0, sizeof(ImuData));
}
...
static void recordFailure(uint8_t idx) {
  ...
  imu_next_allowed_ms[idx] = millis() + (FAIL_COOLDOWN_MS << imu_fail_streak[idx]);
}

static void buildRequest(uint8_t slave_id, uint8_t *out) {
  out[0] = slave_id;
  out[1] = MODBUS_FUNC_READ_HREG;
  out[2] = (MODBUS_REG_START >> 8);
  out[3] = MODBUS_REG_START & 0xFF;
  out[4] = (MODBUS_REG_COUNT >> 8);
...
static bool parseResponse(uint8_t imu_idx, const uint8_t *buf, size_t len) {
  ...
  uint16_t crc_calc = crc16_fast(buf, len - 2);
  uint16_t crc_recv = buf[len - 2] | (buf[len - 1] << 8);
  if (crc_calc != crc_recv) return false;

  const uint8_t *d = buf + 3;
  
  imu_data[imu_idx].acc[0] = (int16_t)((d[0] << 8) | d[1]) * ACC_SCALE;
  imu_data[imu_idx].acc[1] = (int16_t)((d[2] << 8) | d[3]) * ACC_SCALE;
  imu_data[imu_idx].acc[2] = (int16_t)((d[4] << 8) | d[5]) * ACC_SCALE;

  // 四元数在数据末尾8字节
  const uint8_t *q = buf + 3 + RESPONSE_BYTE_COUNT - 8;
  imu_data[imu_idx].quat[0] = (int16_t)((q[0] << 8) | q[1]) * QUAT_SCALE;
  imu_data[imu_idx].quat[1] = (int16_t)((q[2] << 8) | q[3]) * QUAT_SCALE;
  imu_data[imu_idx].quat[2] = (int16_t)((q[4] << 8) | q[5]) * QUAT_SCALE;
  imu_data[imu_idx].quat[3] = (int16_t)((q[6] << 8) | q[7]) * QUAT_SCALE;
...
static void outputData() {
  int len = 0;
  for (uint8_t i = 0; i < NUM_IMUS; ++i) {
    if (i > 0) output_buffer[len++] = ',';
    
    if (imu_data[i].valid) {
      len += sprintf(output_buffer + len, "%d,%.2f,%.2f,%.2f,%.3f,%.3f,%.3f,%.3f",
        IMU_IDS[i],
        imu_data[i].acc[0], imu_data[i].acc[1], imu_data[i].acc[2],
        imu_data[i].quat[0], imu_data[i].quat[1],
        imu_data[i].quat[2], imu_data[i].quat[3]);
    } else {
      len += sprintf(output_buffer + len, "%d,0,0,0,0,0,0,0", IMU_IDS[i]);
    }
  }
  output_buffer[len++] = '\n';
  output_buffer[len] = '\0';
...
static void reportFrequency() {
  ...
  Serial.printf("# %.1fHz (%u cyc, ok=%u, fail=%u, total=%u)\n",
    freq, cycle_count, success_count, fail_count, total_cycles);
...
static inline bool pickNextImu(uint32_t now_ms) {
  for (uint8_t i = 0; i < NUM_IMUS; ++i) {
    uint8_t idx = (current_imu_index + i) % NUM_IMUS;
...
void setup() {
  ...
  Serial.println("# Extreme Optimized Modbus IMU Reader");
  Serial.printf("# IMUs: %d, Timeout: %dms, Silence: %dus\n", 
    NUM_IMUS, RESPONSE_TIMEOUT_MS, BUS_SILENCE_US);
  Serial.printf("# Output mode: %d, interval: %dms\n", OUTPUT_MODE, OUTPUT_INTERVAL_MS);
...
void loop() {
  ...
  if (!waiting_response && busIsIdle() && pickNextImu(now_ms)) {
    response_pos = 0;
  ...
  // 批量读取响应
  while (waiting_response && response_pos < RESPONSE_LEN) {
    ...
    while (avail-- > 0 && response_pos < RESPONSE_LEN) {
      ...
      // 帧同步
      if (response_pos == 0 && b != IMU_IDS[current_imu_index]) continue;
      if (response_pos == 1 && b != MODBUS_FUNC_READ_HREG) { response_pos = 0; continue; }
      if (response_pos == 2 && b != RESPONSE_BYTE_COUNT) { response_pos = 0; continue; }
      
      response_buf[response_pos++] = b;
    }
  }
