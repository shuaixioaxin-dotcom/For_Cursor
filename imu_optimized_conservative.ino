...
static const uint16_t RESPONSE_TIMEOUT_MS = 10;  // 优化：30ms → 10ms
static const uint32_t BUS_SILENCE_US = 200;  // 优化：500μs → 200μs
static const uint32_t TX_ENABLE_DELAY_US = 20;  // 优化：30μs → 20μs
static const uint32_t TX_DISABLE_DELAY_US = 40;  // 优化：60μs → 40μs
...
static const uint8_t CSV_OUTPUT_DIVIDER = 5;  // 优化：每5个周期输出1次
...
static uint8_t csv_output_counter = 0;  // 新增：CSV输出计数器
...
static void outputCycleCsv() {
  // 优化：每CSV_OUTPUT_DIVIDER个周期才输出一次
  csv_output_counter++;
...
void setup() {
  ...
  Serial.println("# IMU Optimized (Conservative) - Target: 200Hz");
