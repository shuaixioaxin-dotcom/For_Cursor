...
static void sendRequest(const uint8_t *request, size_t len) {
  clearRxBuffer();
...
static bool parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len,
  ...
  int16_t ax =
      static_cast<int16_t>((buf[data_start] << 8) | buf[data_start + 1]);
  int16_t ay =
...
static void doBatchProcessing() {
  for (uint8_t i = 0; i < NUM_IMUS; ++i) {
    ...
    size_t len = Serial2.readBytes(response_buf, RESPONSE_LEN);
    if (len == RESPONSE_LEN &&
        parseResponse(IMU_IDS[i], response_buf, len, imu_data[i])) {
      success_count++;
    } else {
      fail_count++;
      zeroImu(imu_data[i]);
