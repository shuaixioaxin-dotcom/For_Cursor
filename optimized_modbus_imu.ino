...
// ================= 优化后的时序参数 =================
static const uint16_t RESPONSE_TIMEOUT_MS = 5;      // 从30ms优化到5ms
static const uint32_t BUS_SILENCE_US = 100;         // 从500us优化到100us
static const uint32_t TX_ENABLE_DELAY_US = 10;     // 从30us优化到10us
static const uint32_t TX_DISABLE_DELAY_US = 20;    // 从60us优化到20us
static const uint32_t FAIL_COOLDOWN_MS = 5;        // 从20ms优化到5ms
static const uint8_t MAX_BACKOFF_SHIFT = 2;        // 从4优化到2，减少退避时间
...
static void reportFrequency() {
  ...
  // 使用sprintf优化输出
  sprintf(output_buffer, "# Update Frequency: %.2f Hz (%lu cycles, ok=%lu, fail=%lu)\n",
