...
// ================= Timing =================
static const uint32_t TARGET_POLL_HZ       = 200;
static const uint32_t POLL_INTERVAL_US     = 1000000UL / TARGET_POLL_HZ;  // 5000 us
static const uint32_t RESPONSE_TIMEOUT_US  = 4000;
...
uint32_t imu_next_poll_us[NUM_IMUS];
...
static void rxPush(uint8_t b) {
  ...
  if (next == rx_tail) {
    rxDropOne();  // drop the oldest on overflow
  }
  rx_ring[rx_head] = b;
  rx_head = next;
}
...
static void recordFailure(uint8_t idx, uint32_t now_us) {
  ...
  imu_next_allowed_us[idx] = now_us + backoff_ms * 1000UL;
}
...
static bool tryExtractFrame(uint8_t slave_id, uint8_t *frame_out) {
  ...
  // Search start within ring buffer
...
static bool pickNextImu(uint32_t now_us) {
  for (uint8_t offset = 0; offset < NUM_IMUS; ++offset) {
    ...
    uint32_t due = imu_next_poll_us[idx];
    if (imu_next_allowed_us[idx] > due) due = imu_next_allowed_us[idx];
    if (timeReached(now_us, due)) {
...
void setup() {
  ...
  uint32_t stagger_us = POLL_INTERVAL_US / NUM_IMUS;
  if (stagger_us == 0) stagger_us = 1;

  for (uint8_t i = 0; i < NUM_IMUS; ++i) {
    zeroImu(imu_data[i]);
    imu_next_allowed_us[i] = 0;
    imu_fail_streak[i] = 0;
    imu_next_poll_us[i] = start_us + (uint32_t)i * stagger_us;
  }
...
void loop() {
  ...
  // 1) Bulk receive: move UART bytes into ring buffer
  while (Serial2.available() > 0) {
    int v = Serial2.read();
    if (v < 0) break;
    last_bus_activity_us = micros();
  ...
  if (waiting_response) {
    ...
    if (tryExtractFrame(slave_id, response_buf)) {
      if (parseResponse(slave_id, response_buf, RESPONSE_LEN, imu_data[current_imu_index])) {
        ...
        recordFailure(current_imu_index, now_us);
      }
      outputImuData();
  ...
  if (!waiting_response && busIsIdle(now_us)) {
    if (pickNextImu(now_us)) {
      clearAllRx();
      sendRequestCached(current_imu_index);
      uint32_t sent_us = micros();
      request_start_us = sent_us;
      imu_next_poll_us[current_imu_index] = sent_us + POLL_INTERVAL_US;
