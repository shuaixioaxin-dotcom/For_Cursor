...
static const uint8_t MAX_RESPONSE_LEN = 16;
...
// ================= Debug =================
#define DEBUG_OUTPUT 1  // 设为0关闭调试输出
...
#if DEBUG_OUTPUT
  Serial.print("# TX[");
  Serial.print(current_phase == PHASE_ACC ? "ACC" : "QUAT");
  Serial.print("]: ");
  printHex(request, MODBUS_REQUEST_LEN);
  Serial.println();
#endif
...
#if DEBUG_OUTPUT
    Serial.print("# ACC len mismatch: got ");
    Serial.print(len);
    Serial.print(" expect ");
    Serial.println(ACC_RESPONSE_LEN);
#endif
    return false;
  }
  if (buf[0] != slave_id || buf[1] != MODBUS_FUNC_READ_HREG) {
#if DEBUG_OUTPUT
    Serial.println("# ACC header mismatch");
#endif
    return false;
  }
  if (buf[2] != ACC_BYTE_COUNT) {
#if DEBUG_OUTPUT
    Serial.print("# ACC byte count mismatch: got ");
    Serial.print(buf[2]);
    Serial.print(" expect ");
    Serial.println(ACC_BYTE_COUNT);
#endif
    return false;
  }
...
#if DEBUG_OUTPUT
    Serial.print("# ACC CRC fail: calc=");
...
static bool parseQuatResponse(uint8_t slave_id, const uint8_t *buf, size_t len, ImuData &out) {
...
#if DEBUG_OUTPUT
    Serial.print("# QUAT len mismatch: got ");
    Serial.print(len);
    Serial.print(" expect ");
    Serial.println(QUAT_RESPONSE_LEN);
#endif
    return false;
  }
  if (buf[0] != slave_id || buf[1] != MODBUS_FUNC_READ_HREG) {
#if DEBUG_OUTPUT
    Serial.println("# QUAT header mismatch");
#endif
    return false;
  }
  if (buf[2] != QUAT_BYTE_COUNT) {
#if DEBUG_OUTPUT
    Serial.print("# QUAT byte count mismatch: got ");
    Serial.print(buf[2]);
    Serial.print(" expect ");
    Serial.println(QUAT_BYTE_COUNT);
#endif
    return false;
  }
  ...
  if (crc_calc != crc_recv) {
#if DEBUG_OUTPUT
    Serial.print("# QUAT CRC fail: calc=");
    Serial.print(crc_calc, HEX);
    Serial.print(" recv=");
    Serial.println(crc_recv, HEX);
#endif
...
void setup() {
  ...
  Serial.println("# Modbus RTU Optimized Read - DEBUG VERSION");
...
void loop() {
  ...
  while (waiting_response && Serial2.available() > 0 && response_pos < expected_response_len) {
    ...
    last_bus_activity_us = micros();
    response_buf[response_pos++] = static_cast<uint8_t>(incoming);
  }
  ...
  if (waiting_response) {
    if (response_pos >= expected_response_len) {
      ...
      Serial.print(current_phase == PHASE_ACC ? "ACC" : "QUAT");
      Serial.print("]: ");
      printHex(response_buf, response_pos);
      Serial.println();
#endif
      ...
      Serial.print("/");
      Serial.print(expected_response_len);
      Serial.print(" bytes: ");
      if (response_pos > 0) {
        printHex(response_buf, response_pos);
      }
