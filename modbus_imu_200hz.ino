...
static constexpr uint32_t RS485_BAUD = 921600;

// ================= Output Configuration =================
// Set to 0 to disable CSV output and use binary frames.
static constexpr bool OUTPUT_BINARY = false;
// Output every N completed cycles to reduce print load.
static constexpr uint8_t OUTPUT_EVERY_N_CYCLES = 1;
...
static constexpr uint8_t MODBUS_FUNC_READ_HREG = 0x03;
static constexpr uint16_t MODBUS_REG_START = 0x0034;
static constexpr uint16_t MODBUS_REG_COUNT = 0x0016;  // 22 registers
static constexpr uint8_t MODBUS_REQUEST_LEN = 8;
static constexpr uint16_t RESPONSE_BYTE_COUNT = MODBUS_REG_COUNT * 2;
static constexpr uint16_t RESPONSE_LEN = 1 + 1 + 1 + RESPONSE_BYTE_COUNT + 2;

// Modbus RTU requires >= 3.5 char times of silence between frames.
static constexpr uint32_t MODBUS_SILENCE_US =
    (35000000UL + RS485_BAUD - 1) / RS485_BAUD;  // 35 bits @ 10 bits/char
static constexpr uint32_t BUS_SILENCE_US = (MODBUS_SILENCE_US < 80) ? 80 : MODBUS_SILENCE_US;

static constexpr uint32_t RESPONSE_TIMEOUT_MS = 5;
static constexpr uint32_t TX_ENABLE_DELAY_US = 5;
static constexpr uint32_t TX_DISABLE_DELAY_US = 5;
static constexpr uint32_t FAIL_COOLDOWN_MS = 20;
static constexpr uint8_t MAX_BACKOFF_SHIFT = 4;

// ================= Data Conversion =================
static constexpr float ACC_SCALE = 0.0048828f;
static constexpr float QUAT_SCALE = 0.0001f;

// ================= Frequency Monitoring =================
static constexpr uint32_t FREQ_REPORT_INTERVAL_MS = 1000;

static_assert(OUTPUT_EVERY_N_CYCLES >= 1, "OUTPUT_EVERY_N_CYCLES must be >= 1");
...
static void sendRequestIndex(uint8_t idx) {
  clearRxBuffer();
  digitalWrite(RS485_DE_RE_PIN, HIGH);
  delayMicroseconds(TX_ENABLE_DELAY_US);
  Serial2.write(request_frames[idx], MODBUS_REQUEST_LEN);
  Serial2.flush();
...
static bool parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len, ImuData &out) {
  ...
  const size_t quat_offset = data_start + (RESPONSE_BYTE_COUNT - 8);
...
static void outputCycleBinary() {
  ...
  Serial.write(NUM_IMUS);
  for (uint8_t i = 0; i < NUM_IMUS; ++i) {
    const uint8_t valid = imu_data[i].valid ? 1 : 0;
    Serial.write(IMU_IDS[i]);
    Serial.write(valid);
    Serial.write(reinterpret_cast<const uint8_t *>(imu_data[i].acc), sizeof(imu_data[i].acc));
    Serial.write(reinterpret_cast<const uint8_t *>(imu_data[i].quat), sizeof(imu_data[i].quat));
  }
}

static void outputCycle() {
  if (OUTPUT_BINARY) {
    outputCycleBinary();
  } else {
    outputCycleCsv();
...
static void advanceImuIndex() {
  ...
  if (current_imu_index >= NUM_IMUS) {
    ...
    if ((cycle_index % OUTPUT_EVERY_N_CYCLES) == 0) {
      outputCycle();
    }
  }
}
...
void setup() {
  ...
  Serial2.setRxBufferSize(RESPONSE_LEN * 2);
  ...
  Serial.println();
  if (OUTPUT_BINARY) {
    Serial.println("# Output: binary frames (0xAA55 + count + id + valid + acc[3] + quat[4])");
  } else {
...
void loop() {
  ...
  if (!waiting_response) {
    if (busIsIdle() && pickNextImu(now_ms)) {
      response_pos = 0;
      sendRequestIndex(current_imu_index);
  ...
  while (waiting_response && available-- > 0 && response_pos < RESPONSE_LEN) {
