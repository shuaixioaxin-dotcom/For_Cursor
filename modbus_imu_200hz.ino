...
static const uint32_t MODBUS_CHAR_BITS = 10;
static const uint32_t BUS_SILENCE_US =
    ((35UL * 1000000UL + RS485_BAUD - 1) / RS485_BAUD) + 10;
...
static const uint32_t CSV_OUTPUT_INTERVAL_MS = 0;
...
static bool parseResponse(uint8_t slave_id,
                          const uint8_t *buf,
                          size_t len,
                          ImuData &out) {
  if (len != RESPONSE_LEN) {
  ...
  if (buf[2] < 8) {
    ...
    int16_t qx =
        static_cast<int16_t>((buf[quat_offset + 2] << 8) | buf[quat_offset + 3]);
    int16_t qy =
        static_cast<int16_t>((buf[quat_offset + 4] << 8) | buf[quat_offset + 5]);
    int16_t qz =
        static_cast<int16_t>((buf[quat_offset + 6] << 8) | buf[quat_offset + 7]);
  ...
  if (current_imu_index >= NUM_IMUS) {
    ...
    if (ENABLE_CSV_OUTPUT) {
      if (CSV_OUTPUT_INTERVAL_MS == 0 ||
          millis() - last_csv_output_ms >= CSV_OUTPUT_INTERVAL_MS) {
        last_csv_output_ms = millis();
        outputCycleCsv();
      }
    }
  }
...
void loop() {
  ...
  if (waiting_response) {
    if (response_pos >= RESPONSE_LEN) {
      bool ok =
          parseResponse(IMU_IDS[current_imu_index], response_buf, response_pos,
