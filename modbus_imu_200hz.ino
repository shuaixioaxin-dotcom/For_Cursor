...
// Timing derived from baud rate (8N1 => 10 bits per char)
static const uint32_t BITS_PER_CHAR = 10;
static const uint32_t CHAR_TIME_US =
    (BITS_PER_CHAR * 1000000UL + RS485_BAUD - 1) / RS485_BAUD;
static const uint32_t BUS_SILENCE_US = CHAR_TIME_US * 4;  // >= 3.5 chars
static const uint32_t TX_ENABLE_DELAY_US = CHAR_TIME_US;  // 1 char
static const uint32_t TX_DISABLE_DELAY_US = CHAR_TIME_US; // 1 char
static const uint32_t RESPONSE_TIMEOUT_US =
    ((RESPONSE_LEN * BITS_PER_CHAR * 1000000UL + RS485_BAUD - 1) / RS485_BAUD) +
    1500; // 1.5 ms margin
...
static const uint32_t CSV_OUTPUT_INTERVAL_MS = 0; // 0 = every cycle
...
static uint32_t last_csv_output_ms = 0;
...
static void sendRequest(uint8_t idx) {
  clearRxBuffer();
...
static bool parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len,
                          ImuData &out) {
  if (len != RESPONSE_LEN) {
    return false;
  }
  ...
  int16_t ax = static_cast<int16_t>((buf[data_start] << 8) |
                                    buf[data_start + 1]);
  int16_t ay = static_cast<int16_t>((buf[data_start + 2] << 8) |
                                    buf[data_start + 3]);
  int16_t az = static_cast<int16_t>((buf[data_start + 4] << 8) |
                                    buf[data_start + 5]);
  ...
  if (buf[2] < 8) {
    ...
    int16_t qw = static_cast<int16_t>((buf[quat_offset] << 8) |
                                      buf[quat_offset + 1]);
    int16_t qx = static_cast<int16_t>((buf[quat_offset + 2] << 8) |
                                      buf[quat_offset + 3]);
    int16_t qy = static_cast<int16_t>((buf[quat_offset + 4] << 8) |
                                      buf[quat_offset + 5]);
    int16_t qz = static_cast<int16_t>((buf[quat_offset + 6] << 8) |
                                      buf[quat_offset + 7]);
...
static void maybeOutputCycleCsv() {
  if (CSV_OUTPUT_INTERVAL_MS == 0) {
    outputCycleCsv();
    return;
  }
  uint32_t now_ms = millis();
  if (now_ms - last_csv_output_ms >= CSV_OUTPUT_INTERVAL_MS) {
    last_csv_output_ms = now_ms;
    outputCycleCsv();
  }
}
...
void loop() {
  ...
  if (waiting_response) {
    if (response_pos >= RESPONSE_LEN) {
      bool ok = parseResponse(IMU_IDS[current_imu_index], response_buf,
