...
static const uint16_t RESPONSE_TIMEOUT_MS = 6;
static const uint32_t CHAR_TIME_US =
    (1000000UL * 10 + RS485_BAUD - 1) / RS485_BAUD;
static const uint32_t BUS_SILENCE_US = 8 * CHAR_TIME_US;
...
// ================= Output Control =================
static const bool ENABLE_CSV_OUTPUT = true;
...
static uint32_t output_cycle_counter = 0;
...
static void sendRequestByIndex(uint8_t idx) {
  clearRxBuffer();
...
static bool parseResponse(uint8_t slave_id, const uint8_t *buf, size_t len,
  ...
  int16_t ay =
      static_cast<int16_t>((buf[data_start + 2] << 8) | buf[data_start + 3]);
  int16_t az =
      static_cast<int16_t>((buf[data_start + 4] << 8) | buf[data_start + 5]);
  ...
  if (buf[2] < 8) {
    ...
    int16_t qw =
        static_cast<int16_t>((buf[quat_offset] << 8) | buf[quat_offset + 1]);
    int16_t qx = static_cast<int16_t>(
        (buf[quat_offset + 2] << 8) | buf[quat_offset + 3]);
    int16_t qy = static_cast<int16_t>(
        (buf[quat_offset + 4] << 8) | buf[quat_offset + 5]);
    int16_t qz = static_cast<int16_t>(
        (buf[quat_offset + 6] << 8) | buf[quat_offset + 7]);
...
static void handleCycleComplete() {
  cycle_count++;
  if (freq_start_ms == 0) {
    freq_start_ms = millis();
  }
  if (ENABLE_CSV_OUTPUT) {
    output_cycle_counter++;
    if (output_cycle_counter >= OUTPUT_EVERY_N_CYCLES) {
...
void loop() {
  if (!waiting_response) {
    ...
    if (busIsIdle() && pickNextImu(now_ms)) {
      response_pos = 0;
      sendRequestByIndex(current_imu_index);
